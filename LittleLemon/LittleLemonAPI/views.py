...
class MenuItemViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": f"Featured status of {str(menuitem.title)} changed to {str(menuitem.featured)}"},
...
class CartView(generics.ListCreateAPIView, generics.DestroyAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": f"{serialized_item.data['menuitem']} successfully added to the cart for {request.user.username}"},
...
# class CartItemView(generics.ListCreateAPIView, generics.RetrieveDestroyAPIView):
#     permission_classes = [IsAuthenticated]
#     serializer_class = CartSerializer
...
class OrderItemView(generics.ListAPIView, generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_permissions(self):
        if self.request.method in ["PUT", "DELETE"]:
            ...
            permission_classes = [IsManager| IsAdmin | IsDeliveryCrewAndOwner | IsCustomerAndOwner]
        return [permission() for permission in permission_classes]
    
    def get_queryset(self):
        # if self.request.user.groups.filter(name="Managers").exists() or self.request.user.is_superuser == True:
        #     return OrderItem.objects.filter(order=self.kwargs["pk"])
        # elif IsDeliveryCrewAndOwner().has_object_permission(self.request, self, Order.objects.get(pk=self.kwargs["pk"])):
        #     return OrderItem.objects.filter(order=self.kwargs["pk"], order__delivery_crew=self.request.user)
        # else:
        #     return OrderItem.objects.filter(order=self.kwargs["pk"], order__user=self.request.user)
    ...
    def list(self, request, *args, **kwargs):
        ...
        if IsDeliveryCrewAndOwner().has_object_permission(self.request, self, order) or IsCustomerAndOwner().has_object_permission(self.request, self, order):
            return Response(serialized_items.data, status=status.HTTP_200_OK)
        elif IsManager().has_permission(self.request, self) or IsAdmin().has_permission(self.request, self):
            return Response(serialized_items.data, status=status.HTTP_200_OK)
        else:
            return Response({"message": "You do not have permission to see this page!"},
...
class ManagerPostView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if username:
            ...
            if user.groups.count() == 0 and user.is_staff == False and user.is_active == True:
...
class DeliveryCrewPostView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if username:
            ...
            return Response(
                {"message": f"{username} successfully added to 'Delivery Crew' group"},
                status=status.HTTP_201_CREATED
            )
...
class DeliveryCrewDeleteView(generics.RetrieveDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": f"{user.username} successfully removed from 'Delivery Crew' group"},
            status=status.HTTP_200_OK
        )
